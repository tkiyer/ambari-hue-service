...
def download_hue():
  ...
  """
  Execute('echo {0} > /usr/lib/python2.7/site-packages/sitecustomize.py'.format(params.hue_env_python))
  ...
  try:
      Execute('groupadd hue && useradd hue -g hue')
  except:
      print " "
  Execute('chown -R hue:hue /usr/local/hue')
  try:
      Execute('mkdir -p {hue_log_dir}')
  except:
      print " "
  Execute('chown -R hue:hue {hue_log_dir}')

  Execute('echo {0} > /usr/local/hue/build/env/lib/python2.7/site-packages/sitecustomize.py'.format(params.hue_env_python))
...
def add_spark_configuration(if_ranger=False, security_enabled=False):
  services_configurations = {}
  services_configurations['livy2-conf'] = {}
  services_configurations['livy2-conf']['livy.server.csrf_protection.enabled'] = 'false'
  add_configurations(services_configurations)
